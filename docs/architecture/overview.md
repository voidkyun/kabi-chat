...
- Infrastructure: Terraform, AWS (アーリーアクセスは単一 instance 構成)
- Authentication: Discord OAuth2 login, JWT
- Database: PostgreSQL

## Repository Layout

モノレポのトップレベル配置は以下を既定とします。

- `frontend/`
  - React SPA と npm 関連ファイルを配置する
- `backend/`
  - Django / DRF と Poetry 関連ファイルを配置する
- `infra/terraform/`
  - Terraform の環境別 entrypoint と module を配置する
- `docs/`
  - アーキテクチャと AI 向け運用ルールの正本を配置する
- `docker-compose.yml`
  - ローカル開発用の共通サービス定義を配置する

以後の実装 issue はこの配置を前提として進めます。
...
- アーリーアクセスでは 10 名以下・月額およそ 20 USD 以下を目安に常設クラウド環境を絞る
